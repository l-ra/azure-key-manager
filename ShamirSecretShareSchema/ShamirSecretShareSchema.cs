...
namespace ShamirSecretShareSchema
{

    /*
     * Copyright © 2017 Coda Hale (coda.hale@gmail.com)
     * C# transposition lra@r73.info
    ...
    class GF256
    {
        ...
        static byte mul(byte a, byte b)
        {
            ...
            return EXP[(uint)(LOG[(uint)(a)]) + (uint)(LOG[(uint)(b)])];
        }

        static byte div(byte a, byte b)
        {
            // multiply by the inverse of b
            return mul(a, EXP[255 - (uint)(LOG[(uint)(b)])]);
        }
        ...
        public static byte[] generate(RandomNumberGenerator random, int _degree, byte x)
        {
            byte[] p = new byte[_degree + 1];

            // generate random polynomials until we find one of the given degree
            do
            {
                random.GetBytes(p);
            } while (degree(p) != _degree);
    ...
    public class Scheme
    {
        ...
        {
            return new Scheme(n, k);
        }

        private Scheme(int n, int k)
        {
         ...
         */
        public Dictionary<int, byte[]> split(byte[] secret)
        {
            ...
            for (int i = 0; i < secret.Length; i++)
            {
                ...
                byte[] p = GF256.generate(random, k - 1, secret[i]);
                for (int x = 1; x <= n; x++)
                {
            ...
            Dictionary<int, byte[]> parts = new Dictionary<int, byte[]>();
            for (int i = 0; i < values.Length; i++)
            {
                parts.Add(i + 1, values[i]);
            }
            return parts;
        }
         ...
         *     lengths
         */
        public byte[] join(Dictionary<int, byte[]> parts)
        {
            checkArgument(parts.Count > 0, "No parts provided");
            //TODO migrate to c#
            //int[] lengths = parts.values().stream().mapToInt(v -> v.length).distinct().toArray();
            //checkArgument(lengths.length == 1, "Varying lengths of part values");
            //byte[] secret = new byte[lengths[0]];
            var x=parts.GetEnumerator();
            x.MoveNext();
            byte[] secret = new byte[x.Current.Value.Length];
            for (int i = 0; i < secret.Length; i++)
            {
